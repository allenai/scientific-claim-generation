...
if __name__ == '__main__':
    ...
    if training_args.do_predict:
        ...
        with open(dm_args.output_predictions, 'wt') as f:
            for context,claim in zip(gen_dset['context'], all_samples):
                gen_claims = tokenizer.decode(claim, skip_special_tokens=True, clean_up_tokenization_spaces=False).split('||')
                f.write(json.dumps({'context': context, 'generated_claims': gen_claims}) + '\n')
                pieces = context.split('||')
                output_csv.append([pieces[0], pieces[1], gen_claims[0]])
                if len(gen_claims) > 1:
                    for c in gen_claims[1:]:
                        output_csv.append(['', '', c])
        output = pd.DataFrame(output_csv, columns=['Context', 'Original Sentence', 'Claim'])
        output.to_csv(f"{dm_args.output_predictions}.csv", index=None)
